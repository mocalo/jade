...
import { getPayAgencyAPI, getPaymentByUserAPI } from '@/services/payAddress'
...
const getOrder = async () => {
  console.log('当前支付信息：', query.payFlag)
  console.log('当前支付信息1：', query.orderId)
  const res = await getMemberOrderByIdAPI(query.orderId)
  console.log('当前order', res.data)
...
const getPayAddress = async () => {
  console.log('当前支付信息：', query.payFlag)
  console.log('当前支付信息2：', query.orderId)
  var payment_type = ''
  if (query.payFlag == 1) {
    payment_type = 'bank'
